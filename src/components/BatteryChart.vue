...
const chartData = computed(() => {
  ...
  const batteryLevels = deviceRecords.value.map((record) => Math.round(record.batteryLevel * 100));
...
<template>
  <div class="bg-white rounded-lg p-4 shadow-sm mb-6">
    ...
    <div class="h-60">
      <Line v-if="deviceRecords.length > 1" :data="chartData" :options="chartOptions" />
